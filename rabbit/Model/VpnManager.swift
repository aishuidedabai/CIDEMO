...
class VpnManager{
    ...
    func updateVPNStatus(_ manager: NEVPNManager) {
        ...
        case .connected:
            ...
            userdefaults.set("连接成功ON", forKey:"YJ判断连接");
            userdefaults.synchronize()
            
            VPNmanagerAPI.success("name" , userid: "test")
        ...
        case .connecting, .reasserting:
            ...
            NotificationCenter.default.post(name: Notification.Name(rawValue: "YJConnecting"),
                                            object: nil, userInfo: nil)
            let userdefaults = UserDefaults.standard
            userdefaults.set("连接中Connecting", forKey:"YJ判断连接");
            userdefaults.synchronize()
        ...
        case .disconnecting:
            ...
            NotificationCenter.default.post(name: Notification.Name(rawValue: "YJDisconnecting"),
                                            object: nil, userInfo: nil)
            let userdefaults = UserDefaults.standard
            userdefaults.set("断开连接中Disconnecting", forKey:"YJ判断连接");
            userdefaults.synchronize()
        ...
        case .disconnected, .invalid:
            ...
            NotificationCenter.default.post(name: Notification.Name(rawValue: "YJoff"),
                                            object: nil, userInfo: nil)
            let userdefaults = UserDefaults.standard
            userdefaults.set("断开连接or连接失败off", forKey:"YJ判断连接");
            userdefaults.synchronize()
        }
...
extension VpnManager{
    ...
    fileprivate func setRulerConfig(_ manager:NETunnelProviderManager){
        ...
        orignConf.providerConfiguration = ["ss_address": userdefaults?["ss_address"] ?? "", "ss_method": userdefaults?["ss_method"] ?? "", "ss_port":userdefaults?["ss_port"] ?? 0, "ss_password": userdefaults?["ss_password"] ?? ""]
